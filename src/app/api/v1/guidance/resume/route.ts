...
import { resolveTarget, reviewResumeText, saveReview } from "@/modules/guidance/resume";
...
export const POST = route(async (request: Request) => {
  ...
  const { review, provider } = reviewResumeText({ text, target });

  const saved = await saveReview({ userId: session.sub, documentId, target, review, provider });
