    ...
    questions: {
      ...
      items: {
        ...
        properties: {
          ...
          probing: { type: "string", description: "What the interviewer is testing. Not the answer." },
          category: {
            type: "string",
            enum: ["OPENER", "TECHNICAL", "BEHAVIOURAL", "SITUATIONAL", "MOTIVATION", "CLOSING"],
          },
          difficulty: { type: "string", enum: ["WARM_UP", "STANDARD", "STRETCH"] },
          skeleton: {
            type: "array",
            minItems: 2,
            maxItems: 5,
            items: { type: "string" },
            description: "Structure for an answer. Never a scripted answer itself.",
          },
        },
        required: ["question", "probing", "category", "difficulty", "skeleton"],
      },
    },
  },
  required: ["questions"],
...
export async function generateInterview(input: {
  ...
  const fallback = buildQuestionSet({ role, round: input.round, count });
  const provider = getProvider();

  const chunks = role
    ? await retrieve({ query: `${role.label} interview responsibilities skills`, entityTypes: ["career"], limit: 4 })
    : [];
  const citations = toCitations(chunks);

  if (!provider.isModelBacked) {
    return { questions: fallback, citations, provider: provider.name, label, grounded: Boolean(role) };
  }

  const result = await provider.structured<{ questions: Draft[] }>({
    schemaName: "generate_interview",
    schema: QUESTIONS_SCHEMA as unknown as Record<string, unknown>,
    maxTokens: 3000,
    system: `Write ${count} interview questions for a candidate preparing for: ${label}.

Round: ${input.round}. ${INTERVIEW_ROUNDS.find((r) => r.value === input.round)?.blurb ?? ""}

<retrieved>
${renderContext(chunks) || "No guide is on file for this role."}
</retrieved>

Rules:
- Ground the role-specific questions in the retrieved guide. If the guide is empty, write general questions and keep them honest rather than inventing specifics about a role you know nothing about.
- 'probing' says what the interviewer is testing for. It is not a hint at the answer.
- 'skeleton' is a shape to hang their own experience on — three or four beats. Never write the answer for them; a memorised answer performs worse in the room than a rough one.
- No trick questions and no brain-teasers. Ask what the work actually requires.
- Mix difficulty. At least one question should be genuinely hard.
- Never imply what a "correct" answer would be worth, or that a good answer gets the job.`,
    messages: [
      {
        role: "user",
        content: `Target: ${label}. Round: ${input.round}. Produce ${count} questions.`,
      },
    ],
  ...
  if (result.usedFallback) {
    return { questions: fallback, citations, provider: result.provider, label, grounded: Boolean(role) };
  }
...
/** Same philosophy as the résumé rulebook: thresholds stated, not hidden. */
...
function deterministicFeedback(
  ...
  return {
    ...
    improvedAnswer: [
      ...
      "This deployment is running without a language-model key, so this is a structural rewrite prompt rather than a rewrite of your specific wording.",
    ].join("\n"),
    followUps: [
...
const FEEDBACK_SCHEMA = {
  ...
  properties: {
    ...
    improvedAnswer: {
      type: "string",
      description: "The candidate's own answer, rewritten. Never a different story.",
    },
    followUps: {
      type: "array",
      maxItems: 4,
      items: { type: "string" },
      description: "Follow-up questions a real interviewer would ask after this answer.",
    },
  },
  required: ["strengths", "gaps", "improvedAnswer", "followUps"],
} as const;
...
export async function gradeAnswer(input: {
  ...
  answer: string;
  targetLabel: string;
}): Promise<{ feedback: InterviewFeedback; provider: string }> {
  const scored = scoreAnswer({ answer: input.answer, question: input.question });
  const fallback = deterministicFeedback(scored, input.question, input.answer);
  const provider = getProvider();

  if (!provider.isModelBacked || input.answer.trim().length < 40) {
    return { feedback: fallback, provider: provider.name };
  }

  const result = await provider.structured<Omit<InterviewFeedback, "score" | "rubric">>({
    schemaName: "grade_answer",
    schema: FEEDBACK_SCHEMA as unknown as Record<string, unknown>,
    maxTokens: 1800,
    system: `You are giving interview feedback for a candidate preparing for: ${input.targetLabel}.

The question was: ${input.question.question}
What it tests: ${input.question.probing}

A rulebook has already scored the answer. Do not restate the scores or contradict them:
${scored.rubric.map((row) => `- ${row.label}: ${row.score}/100 — ${row.comment}`).join("\n")}

Rules:
- 'improvedAnswer' must be a rewrite of THEIR answer, keeping their facts, their example and their voice. Never substitute a different story. Where a number is missing, mark it [how many?] rather than inventing one.
- Do not invent achievements, employers or outcomes they did not mention.
- Be direct about what is weak. Vague encouragement wastes their preparation time.
- Critique the answer, not the person, and never comment on whether they will get the job.
- 'followUps' are questions a real interviewer would ask *because of what they just said*.`,
    messages: [{ role: "user", content: input.answer.slice(0, 8000) }],
    fallback: () => fallback,
  });

  if (result.usedFallback) return { feedback: fallback, provider: result.provider };

  return {
    feedback: {
      // The numbers stay the rulebook's, always.
      score: scored.score,
      rubric: scored.rubric,
      strengths: result.value.strengths?.length ? result.value.strengths : fallback.strengths,
      gaps: result.value.gaps?.length ? result.value.gaps : fallback.gaps,
      improvedAnswer: result.value.improvedAnswer || fallback.improvedAnswer,
      followUps: result.value.followUps?.length ? result.value.followUps : fallback.followUps,
    },
    provider: result.provider,
  };
}
