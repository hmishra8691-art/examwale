  ...
  properties: {
    rewrites: {
      ...
      items: {
        type: "object",
        properties: {
          before: { type: "string", description: "A line copied verbatim from the résumé." },
          after: { type: "string", description: "The rewritten line." },
          why: { type: "string", description: "One sentence on what changed and why." },
        },
    ...
    fixes: {
      ...
      items: {
        ...
        properties: {
          ...
          issue: { type: "string" },
          action: { type: "string", description: "Something the user can do today." },
        },
        required: ["priority", "issue", "action"],
      },
    },
  },
  required: ["rewrites", "strengths", "fixes"],
} as const;

export async function reviewResumeText(input: {
  text: string;
  target: ReviewTarget;
}): Promise<{ review: ResumeReviewShape; provider: string }> {
  const scored = scoreResume(input.text, input.target);
  const deterministic = deterministicAdvice(scored, input.target);
  const provider = getProvider();

  const assemble = (
    advice: Pick<ResumeReviewShape, "rewrites" | "strengths" | "fixes">,
    providerName: string,
    ...
    review: {
      ...
      limitations:
        providerName === "anthropic"
          ...
          : [
              ...LIMITATIONS,
              "This deployment is running without a language-model key, so the rewrites here are rule-generated rather than written for your specific wording.",
            ],
    },
    provider: providerName,
  });

  if (!provider.isModelBacked || input.text.trim().length < 120) {
    return assemble(deterministic, provider.name);
  }

  const result = await provider.structured<Pick<ResumeReviewShape, "rewrites" | "strengths" | "fixes">>({
    schemaName: "review_resume",
    schema: REVIEW_SCHEMA as unknown as Record<string, unknown>,
    maxTokens: 2400,
    system: `You are reviewing a résumé for ExamWale. The numeric scoring has already been done by a rulebook and is not yours to change — your job is the prose and the rewrites.

Target role: ${input.target.label}
${input.target.skills.length ? `Skills this role calls for: ${input.target.skills.join(", ")}` : "No specific target role was chosen."}

What the rulebook already found:
- ${scored.facts.wordCount} words, ${scored.facts.bulletCount} bullets, ${scored.facts.quantifiedBullets} of them quantified.
- ${scored.facts.headingsFound} standard sections detected.
- Contact: ${scored.facts.hasEmail ? "email present" : "NO EMAIL"}, ${scored.facts.hasPhone ? "phone present" : "NO PHONE"}.
${scored.missing.length ? `- Target skills not mentioned anywhere: ${scored.missing.join(", ")}` : ""}

Rules:
- Every 'before' string must be copied verbatim from the résumé text. If you cannot quote it exactly, do not include that rewrite.
- Rewrite the person's actual line. Do not substitute a generic example. Where a number is needed and you do not have one, leave a clearly marked placeholder like [X%] rather than inventing a figure.
- Do not claim an achievement the résumé does not state. You are improving how it reads, not what it says.
- 'fixes' are actions for today, in priority order. Be specific to this document.
- Critique the document, never the person. No comments on their career choices, their employer, or their prospects.
- Do not tell them they will get the job. You have no idea.`,
    messages: [{ role: "user", content: input.text.slice(0, 20_000) }],
    fallback: () => deterministic,
  });

  if (result.usedFallback) return assemble(deterministic, result.provider);

  // Rewrites whose `before` isn't actually in the résumé are dropped: an
  // invented quotation is the one failure mode that would make this feature
  // worse than useless.
  const haystack = input.text.replace(/\s+/g, " ");
  const rewrites = (result.value.rewrites ?? []).filter((entry) =>
    haystack.includes(entry.before.replace(/\s+/g, " ").trim()),
  );

  return assemble(
    {
      rewrites: rewrites.length ? rewrites : deterministic.rewrites,
      strengths: result.value.strengths?.length ? result.value.strengths : deterministic.strengths,
      fixes: result.value.fixes?.length ? result.value.fixes : deterministic.fixes,
    },
    result.provider,
  );
}
